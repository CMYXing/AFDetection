...
from read_data import read_extracted_short_data

import augmentation as aug
from network import cnn_lstm_network as cln
...
def training():
    ...
    # Parameter setting
    model_name = "cnn_feed_lstm_" + time.strftime("%m-%d-%H:%M:%S") + ".pkl"
    num_epochs = 200
    batch_size = 256
    learning_rate = 1e-4
    weight_decay = 1e-3
    initial_path = "model/cnn_feed_lstm_06-14-00:21:06.pkl"
    augmentation = True
    ...
    cnn_architecture = "cnn_concat_lstm" if "cnn_concat_lstm" in model_name else "cnn_feed_lstm"
    model = cln.load_network(device, architecture=cnn_architecture, path=initial_path, **cnn_lstm_params)
    model = model.to(device)

    # Logging
    handler = logging.FileHandler("logging/" + cnn_architecture + "_" + time.strftime("%m-%d-%H:%M:%S") + ".txt")
    ...
    logger.info("Model architecture: " + cnn_architecture)
    logger.info("Bidirectional: " + str(cnn_lstm_params['bidirectional']))
    logger.info("Num of features: " + str(cnn_lstm_params['num_features']))
    ...
    # load data
    X_train, Y_train, _, class_weights = read_extracted_short_data(folder=paths.parsed_train_folder, mode="entire")
    X_test, Y_test, Name_list, _ = read_extracted_short_data(folder=paths.parsed_test_folder, mode="entire")
...
def get_cnn_lstm_feature(data=None, names=None, name_list=None, model_architecture='cnn_feed_lstm', model_name=None):
    ...
    cnn_lstm_params['num_features'] = 32
    cnn_lstm_params['bidirectional'] = True
    model = cln.load_network(device, architecture=model_architecture, path=model_name, **cnn_lstm_params)
    model = model.to(device)

    print("(1) Predicting the features using the pre-trained CNN_LSTM network...")
    temp_features = []
    num_of_data = len(data)
    minibatch_data = []
    for i in range(num_of_data):
        minibatch_data.append(data[i])
        if (i % 1024 == 0 or i == (num_of_data - 1)) and i != 0:
    ...
    for i in range(len(features)):
        cnn_lstm_features.append(features[i])

    cnn_lstm_features = np.array(cnn_lstm_features)
    return cnn_lstm_features


def get_cnn_lstm_predition(data=None, names=None, name_list=None, model_architecture='cnn_feed_lstm', model_name=None):
    ...
    print("(1) Predicting the predictions using the pre-trained CNN_LSTM network...")
    temp_predictions = []
    num_of_data = len(data)
    minibatch_data = []
    for i in range(num_of_data):
    ...
    for i in range(len(predictions)):
        cnn_lstm_predictions.append(predictions[i])

    cnn_lstm_predictions = np.array(cnn_lstm_predictions)
    return cnn_lstm_predictions
